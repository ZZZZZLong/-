...
namespace Unity.Cn.Multiverse
{
    ...
    public class MultiverseAlphaSdk : MultiverseSdk
    {
        ...
        public async Task<bool> PlayerConnect(string id)
        {
            ...
            return await SendRequestAsync(    SidecarAddress, "/alpha/player/connect", json).ContinueWith(task => task.Result.ok);
        }
        ...
        {
            string json = JsonUtility.ToJson(new Player(playerId: id));
            return await SendRequestAsync(SidecarAddress,"/alpha/player/disconnect", json).ContinueWith(task => task.Result.ok);
        }
